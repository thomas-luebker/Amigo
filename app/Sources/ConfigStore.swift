...
enum ConfigStore {
    ...
    /// Settings silently reverted — every machine change was affected, not
    /// just the obvious ones. The crash-loop protection is unaffected: if
    /// the change really does break the boot, the app is gone before this
    /// fires.
